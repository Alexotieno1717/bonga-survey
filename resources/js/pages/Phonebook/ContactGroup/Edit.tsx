...
export default function Edit() {
    const { contactGroup } = usePage().props as unknown as {
        contactGroup: ContactGroupPayload;
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mx-auto flex h-full w-full max-w-2xl flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="rounded-xl border bg-white p-6 shadow-sm">
                    <h1 className="text-2xl font-semibold">Edit Contact Group</h1>
                    <p className="pb-6 text-sm text-slate-600">Update this contact group.</p>

                    <form
                        className="space-y-6"
                        onSubmit={(event) => {
                            event.preventDefault();
                            put(route('contactgroup.update', contactGroup.id), {
                                onSuccess: () => {
                    ...
                    >
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <Button type="submit" disabled={processing}>
                                {processing ? 'Updating...' : 'Update Contact Group'}
                            </Button>
                        </div>
                    </form>
                </div>
